// File: route.ts
// Description: Handles user/search operations with secure JWT validation
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!decoded) {
      console.error('[GET user/search] Error: Unauthorized - Invalid or missing token');
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!decoded) {
      console.error('[POST user/search] Error: Unauthorized - Invalid or missing token');
      const response = NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
      return addCorsHeaders(response, req.headers.get('origin'));
    }
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (!decoded) {
      console.error('[PUT user/search] Error: Unauthorized - Invalid or missing token');
      const response = NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
      return addCorsHeaders(response, req.headers.get('origin'));
    }
    ...
    console.error('[PUT user/search] Error:', error);
    const response = NextResponse.json({ error: 'Internal server error' }, { status: 500 });
    return addCorsHeaders(response, req.headers.get('origin'));
  } finally {
    await prisma.$disconnect();
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!decoded) {
      console.error('[DELETE user/search] Error: Unauthorized - Invalid or missing token');
      const response = NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
      return addCorsHeaders(response, req.headers.get('origin'));
    }
